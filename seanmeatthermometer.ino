...
void drawTemps() { //main screen
  ...
  img.setCursor(1,231);
  img.print(WiFi.localIP()); //draw the IP address
  img.setTextDatum(BR_DATUM); //draw text from the bottom right


  
  //### Battery icon ###
  if (setIcons) { //draw the icons if that setting is on
    img.drawRect(214,230,20,9,cmap[setFGC]);
    img.fillRect(214,230,barx,9,cmap[setFGC]); //draw the battery strength with 'barx' width
  ...
  else { //draw the raw wifi/bat info if that setting is on
  ...
  if (animpos > 140) {animpos = 100;} //make it wrap around
...
void drawCalib(){  //if we're in calibration mode
  ...
  img.setTextSize(2);
   ...
   dallasString = String(tempC, 1) + " C, A0: " + String(ADSToOhms(adc0));  //draw the calibration probe temp and meat probe resistance in ohms
  ...
  if ((tempC >= 30.0) && (tempC <= 30.2)) { 
    ...
    img.drawString(temp1string, 10,40);  //draw those readings
...
void drawSettings() {  //if we're in settings mode
  ...
  img.setTextColor(TFT_WHITE);
  if (setIcons > 1) {setIcons = 0;}  //don't let settings exceed their maximums
...
void setup() {  //do this on bootup
  ...
  digitalWrite(PWR_LED_PIN, HIGH); //turn on the red LED
  ...
  if ((!digitalRead(button1) && !digitalRead(button2)) || !wm.getWiFiIsSaved()){  //If either both buttons are pressed on bootup OR no wifi info is saved
    ...
    if(!res) {  //if the wifi manager failed to connect to wifi
        ...
        digitalWrite(PWR_LED_PIN, HIGH);  //flash the red power LED 
        delay(250);
  ...
  digitalWrite(PWR_LED_PIN, HIGH); //make sure red power LED is still on
  initSPIFFS();

  temperatureSensors.begin(NonBlockingDallas::resolution_12, TIME_INTERVAL);  //fire up the DS18B20 calibration probe system
  if (temperatureSensors.getSensorsCount() > 0)  //check and see if the probe is connected, if it is,
  {
...
void loop() {  //do this all the time, main CPU loop
  ...
  every(10000) {       //every 10 seconds
    ...
    barx = mapf (volts2, 3.5, 4.0, 0, 20); //update the battery icon length
  ...
  every(5){ //every 5 milliseconds, so we don't waste battery power
    ...
    if (!calibrationMode) {  //if it's not calibration town
      if (setLEDmode == 0) {analogWrite(PWR_LED_PIN, 0);}  //configure the red power LED based on settings
    ...
    else {drawCalib();} //else draw the calibration screen
  }
